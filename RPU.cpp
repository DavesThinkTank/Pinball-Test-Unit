...
void RPU_PushToSolenoidStack(byte solenoidNumber, byte numPushes, boolean disableOverride, byte relay) {
  if (solenoidNumber >= RPU_NUM_SOLENOIDS && relay == 99) return;
  if (solenoidNumber >= 2 * RPU_NUM_SOLENOIDS) return;

  if (solenoidNumber >= RPU_NUM_SOLENOIDS) {
    RPU_SetLampState(relay, 1, 0, 0);
    solenoidNumber = solenoidNumber - RPU_NUM_SOLENOIDS;
  }
  else
    RPU_SetLampState(relay, 0, 0, 0);
...
void RPU_UpdateTimedSolenoidStack(unsigned long curTime, byte relay) {
  for (int count=0; count<TIMED_SOLENOID_STACK_SIZE; count++) {
    if (TimedSolenoidStack[count].inUse && TimedSolenoidStack[count].pushTime<curTime) {
      RPU_PushToSolenoidStack(TimedSolenoidStack[count].solenoidNumber, TimedSolenoidStack[count].numPushes, TimedSolenoidStack[count].disableOverride, relay);
...
#if (RPU_MPU_ARCHITECTURE<10)
void RPU_SetDisplayCredits(int value, boolean displayOn, boolean showBothDigits, boolean sixdigits) {
if (sixdigits) {
  ...
  byte enableMask = DisplayDigitEnable[4] & (sixdigits ? 0x60 : 0x30);

  if (displayOn) {
    if (value>9 || showBothDigits) enableMask |= (sixdigits ? 0x0C : 0x06); 
    else enableMask |= (sixdigits ? 0x04 : 0x02);
  }

  DisplayDigitEnable[4] = enableMask;
}

void RPU_SetDisplayBallInPlay(int value, boolean displayOn, boolean showBothDigits, boolean sixdigits) {
if (sixdigits) {
  ...
  byte enableMask = DisplayDigitEnable[4] & (sixdigits ? 0x0C : 0x06);

  if (displayOn) {
    if (value>9 || showBothDigits) enableMask |= (sixdigits ? 0x60 : 0x30);
    else enableMask |= (sixdigits ? 0x20 : 0x10);
  }
...
void RPU_CycleAllDisplays(unsigned long curTime, byte digitNum, boolean sixdigits) {
  ...
  if (digitNum!=0) {
    if (!sixdigits) {
        ...
        displayBlank = (0x40)>>((digitNum-1)%6); // xxx changed from 0x20, because RPU set for 7 digit
        if (digitNum == 31) { // highest value for 6-digit displays with 7-digit BIP Cred, gives display 5 and digit 0, s/b display 4 and digit 7.
          displayNumToShow = 4;
          displayBlank = 0x40>>6;
        }
    }
...
// Dave's Think Tank - Set a single credit digit to flash
void RPU_SetDigitFlashCredits(int digit, unsigned long curTime, int period, boolean sixdigits) {
  if (period) {
    if ((curTime/period)%2) {
      DisplayDigitEnable[4] = 108>>(!sixdigits);
    } else {
      DisplayDigitEnable[4] = (108 - (8 - digit * 4))>>(!sixdigits);
    }
  }
}


// Dave's Think Tank - Set a single ball-in-play digit to flash
void RPU_SetDigitFlashBallInPlay(int digit, unsigned long curTime, int period, boolean sixdigits) {
  if (period) {
    if ((curTime/period)%2) {
      DisplayDigitEnable[4] = 108>>(!sixdigits);
    } else {
      DisplayDigitEnable[4] = (108 - (64 - digit * 32))>>(!sixdigits);
    }
  }
}
